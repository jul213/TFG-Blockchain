...
Sistema de gestión universitaria con seguridad, trazabilidad blockchain, observabilidad SOC y analítica académica.
...
- SPA HTTPS: `https://localhost`
- API health: `https://localhost/api/health-check`
- Grafana SOC: `http://localhost:3002`
- Prometheus: `http://localhost:9090`
- Alertmanager: `http://localhost:9093`

## Capacidades Implementadas

- JWT, MFA compartido opcional para roles privilegiados y RBAC por permiso.
- `request_id` end-to-end desde frontend/API hasta logs, Postgres y auditoría blockchain.
- Errores normalizados con `code`, `message`, `details` y `requestId`.
- Auditoría de login, MFA, escritura de notas, exportaciones y cambios de rol.
- Cruce `grades` + `audit_events` con `blockchain_hash`, `db_id`, `request_id`, bloque y gas.
- Mini explorer con filtros por bloque, hash, dirección, gas y evento.
- Verificación de integridad Postgres vs blockchain.
- SOC con Prometheus, Loki, Grafana, Alertmanager y Discord relay enriquecido.
- Scripts de backup/restore PostgreSQL y snapshot de artefactos de contratos.

## Documentación Técnica

- OpenAPI: `docs/openapi.yaml`
- Arquitectura, trust boundaries, RBAC y MITRE: `docs/architecture.md`
- Runbooks SOC: `docs/runbooks.md`

## Operación

```bash
scripts/backup_postgres.sh
scripts/restore_postgres.sh backups/postgres-YYYYmmddTHHMMSSZ.sql
scripts/snapshot_contracts.sh
```
...
Contenido anterior conservado debajo por si era documentación importada.
